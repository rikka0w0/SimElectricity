...
public final class EnergyNet {
	...
	/*
	 * private Map<Integer, Integer> getNeighboringNodesFromDb(int x, int y, int
	 * z){ Map<Integer, Integer> result = new HashMap(); ResultSet rs =
	 * Db.getNeighboringNodes(x, y ,z); try { while (rs.next()) {
	 * result.put(rs.getInt("node"), rs.getInt("resistance"));
	 * System.out.println(rs.getString("node")); } } catch (SQLException e) { //
