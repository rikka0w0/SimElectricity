...
public class Dcs_sqr {
    ...
    private static boolean cs_vcount(Dcs_common.Dcs A, Dcss S) {
     ...
     */
    public static Dcss cs_sqr(int order, Dcs_common.Dcs A, boolean qr) {
        int n, k, post[];
        Dcss S;
        boolean ok = true;
        ...
        if (qr) /* QR symbolic analysis */ {
            Dcs_common.Dcs C = order > 0 ? Dcs_permute.cs_permute(A, null, S.q, false) : A;
            ...
            ok = C != null && S.parent != null && S.cp != null && Dcs_sqr.cs_vcount(C, S);
            if (ok)
