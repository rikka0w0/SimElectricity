...
public class TransmissionTowerRenderHelper{
    ...
    private void findVirtualConnection(TileEntity tileEntity, int neighborX, int neighborY, int neighborZ, double[] from, double[] to){   
		...
		getRotatedXZ(curInsulatorXZ1, curRotation, tileEntity.getPos().getX(), tileEntity.getPos().getZ());
		
		if (insulatorPositionArray.length > 9){
			double[] curInsulatorXZ2 = new double[] {insulatorPositionArray[12], insulatorPositionArray[14]};
			getRotatedXZ(curInsulatorXZ2, curRotation, tileEntity.getPos().getX(), tileEntity.getPos().getZ());
		...
		transformCoord(from, curRotation, tileEntity.getPos().getX(), tileEntity.getPos().getZ());
		transformCoord(to, curRotation, neighborX, neighborZ);
		from[1] += tileEntity.getPos().getY();
		from[4] += tileEntity.getPos().getY();
		from[7] += tileEntity.getPos().getY();
		to[1] += neighborY;
		to[4] += neighborY;
		to[7] += neighborY;
    ...
    private void findConnection(TileEntity neighbor, double[] from, double[] to){    	
		...
		double[] curInsulatorXZ1 = new double[] {insulatorPositionArray[3], insulatorPositionArray[5]};
		getRotatedXZ(curInsulatorXZ1, rotation, te.getPos().getX(), te.getPos().getZ());
		
		double[] neighborInsulatorXZ1 = new double[] {neighborInsulatorPositionArray[3], neighborInsulatorPositionArray[5]};
		getRotatedXZ(neighborInsulatorXZ1, neighborRotation, neighbor.getPos().getX(), neighbor.getPos().getZ());
		
		double[] curInsulatorXZ2 = null;
		if (insulatorPositionArray.length > 9){
			curInsulatorXZ2 = new double[] {insulatorPositionArray[12], insulatorPositionArray[14]};
			getRotatedXZ(curInsulatorXZ2, rotation, te.getPos().getX(), te.getPos().getZ());
		}
		...
		if (neighborInsulatorPositionArray.length > 9){
			...
			getRotatedXZ(neighborInsulatorXZ2, neighborRotation, neighbor.getPos().getX(), neighbor.getPos().getZ());
		}
		...
		//Transform to 'Real' MC coordinates
		transformCoord(from, rotation, te.getPos().getX(), te.getPos().getZ());
		transformCoord(to, neighborRotation, neighbor.getPos().getX(), neighbor.getPos().getZ());
		from[1] += te.getPos().getY();
		from[4] += te.getPos().getY();
		from[7] += te.getPos().getY();
		to[1] += neighbor.getPos().getY();
		to[4] += neighbor.getPos().getY();
		to[7] += neighbor.getPos().getY();
    ...
    public void updateRenderData(BlockPos pos1, BlockPos pos2) {
    	...
    	updateRenderData(x1, y1, z1, x2, y2, z2);
    }
    
    private void updateRenderData(int x1, int y1, int z1, int x2, int y2, int z2){		
		rotation = ((ISETransmissionTower)te).getRotation()*45 - 90;
        
		TileEntity neighbor1 = te.getWorld().getTileEntity(new BlockPos(x1, y1, z1));
		TileEntity neighbor2 = te.getWorld().getTileEntity(new BlockPos(x2, y2, z2));
